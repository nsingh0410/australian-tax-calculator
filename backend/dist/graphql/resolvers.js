...
const calculator = new tax_calculator_1.TaxCalculator(taxRateService);
exports.resolvers = {
    Query: {
        ...
        taxBrackets: async (_, { year }) => {
            try {
                const connection = await db.connect();
                const [rows] = await connection.execute('SELECT * FROM tax_brackets WHERE income_year = ? ORDER BY bracket_order', [year]);
                return rows.map(row => ({
                    id: row.id,
        ...
        calculateTax: async (_, { income, year }) => {
            try {
                if (income < 0) {
        ...
        allTaxBrackets: async () => {
            try {
                const connection = await db.connect();
                const [rows] = await connection.execute('SELECT * FROM tax_brackets ORDER BY income_year, bracket_order');
                return rows.map(row => ({
                    id: row.id,
                    incomeYear: row.income_year,
                    bracketOrder: row.bracket_order,
    ...
    Mutation: {
        ...
        updateTaxBracket: async (_, { id, input }) => {
            try {
                const connection = await db.connect();
                await connection.execute('UPDATE tax_brackets SET min_income = ?, max_income = ?, tax_rate = ?, description = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [input.minIncome, input.maxIncome === Infinity ? 999999999.99 : input.maxIncome, input.taxRate, input.description, id]);
        ...
        deleteTaxYear: async (_, { year }) => {
            try {
                const connection = await db.connect();
                const [result] = await connection.execute('DELETE FROM tax_brackets WHERE income_year = ?', [year]);
